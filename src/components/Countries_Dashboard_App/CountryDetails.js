...
class CountryDetails extends React.PureComponent {
  ...
  componentDidMount() {
    ...
    let eachCountry = countries.find(function(eachitem) {
  ...
  navigateBack = () => {
    if (this.backButtonObjects.length > 1) {
      ...
      this.setState({ 
        eachCountry: this.backButtonObjects[this.backButtonObjects.length - 1][0] 
      });
    }
    else {
      this.navigateHome();
  ...
  eachCountryBorderName = (border) => {
    const {countries} = this.props
    let eachBorderName = countries.filter(function(filterCountry) {
...
export default withRouter(FetchingDataComponent(CountryDetails));
